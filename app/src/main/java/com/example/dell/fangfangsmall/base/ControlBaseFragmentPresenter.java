...
public class ControlBaseFragmentPresenter<T extends UiView> extends BasePresenter<T> {
    /**
     * 是否来电
     */
    ...
    public ControlBaseFragmentPresenter(T mView) {
        super(mView);
    }
    ...
    public boolean initview() {
        mIncomingCall = ((Fragment) mView).getActivity().getIntent().getBooleanExtra(Constant.ACTION_CALLBACKING, false);
        mCallType = (ECVoIPCallManager.CallType) ((Fragment) mView).getActivity().getIntent().getSerializableExtra(ECDevice.CALLTYPE);
        return false;
    }
